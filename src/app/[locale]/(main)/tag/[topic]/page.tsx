import MediumStyleCard from "@/app/[locale]/(main)/tag/[topic]/_component/MediumStyleCard";
import { AddTopic } from "@/app/[locale]/(main)/tag/[topic]/_component/AddTopic";
...
export default async function TagDetailsPage({ params: _params, searchParams: _searchParams }: PageProps) {
  const params = await _params;
  ...
  return (
    <>
      <Text className={"heading1 min-h-[30px] font-medium capitalize md:min-h-[52px] md:text-[42px]"}>
        {topic.split("-").join(" ")}
      </Text>
      <Text className={"body4 md:body3 mb-[24px] mt-[12px] md:mt-[16px]"}>{t("result_sites_cnt", { count: 10 })}</Text>
      <AddTopic />
      <Row className={"mb-[100px] mt-[40px] w-full flex-wrap text-center"}>
        <div className="grid w-full gap-8 custom-680:grid-cols-2 ls:grid-cols-3">
