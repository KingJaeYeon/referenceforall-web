      ...
      <Row className={"flex flex-wrap"}>
        {!readonly && (
          <Row className={"mb-6 w-full items-center gap-[6px]"}>
            ...
            />
            <Col className={"w-[20%] items-end gap-4 md:flex-row-reverse md:justify-start"}>
              <button className={"body6 text-description hover:text-foreground"}>Done</button>
        ...
        <Link href={`/site/${site.id}`} className={"w-full"}>
          <Row className={"w-full"}>
            <Col className={"w-full"}>
              <h2 className={"heading1 ellipsisLine2 max-h-[90px] font-semibold md:text-[28px]"}>{site.name}</h2>

              <Row className={"pt-[8px]"}>
                <h3 className={"body4 md:body3 ellipsisLine2 min-h-[24px] text-gray-500 md:min-h-[40px]"}>
                  {site.description}
                </h3>
              </Row>
              <Row className={"mt-[10px] hidden h-[48px] max-w-[512px] items-center justify-between md:flex"}>
        ...
        {!isLast && <Row className={"mt-[20px] w-full border-b border-gray-300"} />}
      </Row>
